...
export default function ModeratorHistoryPage() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        limit: '20',
        moderatorId: session?.user?.id || '' // Filtrer par le modérateur connecté
      })
  ...
  }, [currentPage, searchTerm, actionFilter, startDate, endDate, session?.user?.id, fetchHistory])
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Mon Historique de Modération</h1>
        <p className="text-gray-600">Consultez toutes vos actions de modération effectuées</p>
      </div>
      ...
      <div className="mb-6">
        <p className="text-sm text-gray-600">
          {total} action{total > 1 ? 's' : ''} effectuée{total > 1 ? 's' : ''}
        </p>
      </div>
